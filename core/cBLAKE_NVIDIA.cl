...
void KMAC(unsigned char* in, ulong* out, int* inputLen, int* outputLen, unsigned char* key, int* keyLen, unsigned char* customization, int* customizationLen) {
	...
	/*for(int i=0;i<8;i++){
		printf("\nfinish cshake update data b0 %i %016llx\n",i,ctx_sha3.u.wide[i]);
	}*/
	ulong* t3Block = (ulong*) t3;

	cshake_update(&ctx_sha3, t3Block,136*2, 1600-512,  0x04, -2 );
...
void blake2b(unsigned char* block, ulong* output, unsigned int* inputLen, unsigned char* inputKey, unsigned char* keyLen) {
	...
	for(int i=0;i<16;i++){
		...
		if(i <= 3){
			ulong* keyinp = DEC64LE((inputKey+(i*8)));//dec64le_aligned((unsigned char*)(inputKey+(i*8)));
			if(debugBlake == 1) {
	...
	ulong v[16] = { 0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1,
	                0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179,
	                0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1,
	                0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0xe07c265404be4294, 0x5be0cd19137e2179 };

	sph_u32* word0Padding[8];
	...
	ulong t = barf; //padding on first 64 bit word
	v[0] ^= barf;
...
kernel void cBlake(__constant unsigned char* in, __global ulong* out, __constant int* inputLen, __constant int* outputLen, __constant unsigned char* key, __constant int* keyLen, __constant ulong* hashTarget, __global int* hasFoundNonce, __global ulong* nonceOutput, __constant ulong* nonceBaseAsLong){
	...
	hashTarget64[0] = (ulong*) htz;//swapLong((ulong*) htz);
	*/
	//ulong htz2 = (ulong*) hashTarget[1];
	//hashtarget64[1] = (ulong*) hashTarget[1];
	...
	for(int i=0;i<32;i++){
		...
		if(i < 8){
			nonceIn[i] = swapLong((ulong*) nonceBaseAsLong[i]);
			//nonceIn[i] = swapLong((ulong*) nonceIn[i]);
		}
	}
	...
	//for(int jj=0;jj<clMinerThreads;jj++){
		//if(hasFound == 0){
			...
			/*if(nonceLong == 0xFFFFFFFFFFFFFFFF){
				...
				printf("\n overflowing nonce??? %016llx",nonceLong);
			}
			else{
				nonceOffset = 0;
			...
			nonceLong = (ulong*) ( nonceLong + (0x0000000000000001 * nonceAdd)  );
			//printf("\n new noncelong %016llx",nonceLong);
			//nonceLong = 0x0000000000576193; //hard code a nonce if you want to debug, just make sure to stop incrementing on next line.
			/*if(nonceLong >= 0x00000000FFFFFFFF){
				printf("\n nonce in %016llx noncelong %016llx nonceAdd %016llx gid %i lid %i jj %i",nonceIn[0],nonceLong,nonceAdd,gid,lid,jj);
			}*/
			...
			blakeBuffer = (char*) blakeInput;
			blakeBuffer += 0;

			blake2b(input,outputBuf,(int*) 512, (char*) blakeBuffer,(ulong) 32); //64 being # of bytes incoming from blakeIn

			ulong bufOutCompare = swapLong((ulong*) outputBuf[0]);
			ulong ht64Compare = hashTarget[0];
			ulong ht64c2 = hashTarget[1];
			ulong bufComp64c2 = swapLong((ulong*) outputBuf[1]);

			//ht64Compare >>= 8;
			//test really hard full nonces
			//ht64Compare >>= 24;
			/*bufOutCompare = 0x000000000000654d;
			bufComp64c2 = 0xb300000000000000;*/
			if( bufOutCompare <= ht64Compare){
				//ok but first we need to see if there's a second ulong target
				int shouldContinue = 1;
				if( (ht64Compare == 0x0) || (ht64c2 > 0x0) ){
					//ok first target is fully 0, lets check 2nd char..
					if(bufComp64c2 > ht64c2){
						shouldContinue = 0;
					}

				}
				if(shouldContinue == 1){
					//printf("\n comparison is golden! %016llx %016llx %016llx %016llx",bufOutCompare,ht64Compare,bufComp64c2,ht64c2);
					hasFoundNonce[0] = (int) 1;
					hasFound = 1;
					//printf("\n gold found %016llx %016llx",nonceLong,nonceIn[0]+nonceAdd);
					#pragma unroll
					for(int i=0;i<8;i++){
					...
					if(nonceOffset == 0){
						nonceOutput[0] = swapLong((ulong*) nonceLong);
						nonceOutput[1] = 0x0;
						nonceOutput[2] = 0x0;
					}
					else if(nonceOffset == 4){
						nonceOutput[0] = nonceIn[0];
						nonceOutput[1] = swapLong((ulong*) nonceLong);
						nonceOutput[2] = 0x0;
					}
					else if(nonceOffset == 8){
						nonceOutput[0] = nonceIn[0];
						nonceOutput[1] = nonceIn[1];
						nonceOutput[2] = swapLong((ulong*) nonceLong);
					}
				}
			}
		//}
